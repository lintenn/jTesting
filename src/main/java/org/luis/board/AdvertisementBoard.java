...
public class AdvertisementBoard {
  ...
  public void publish(Advertisement advertisement,
    ...
    if (numberOfPublishedAdvertisements() >= MAX_BOARD_SIZE) {
      throw new FullBoardException("The board is full");
    }

    Advertisement possibleExistingAdvertisement = this.findByTitle(advertisement.title);

    if (possibleExistingAdvertisement == null || !possibleExistingAdvertisement.advertiser.equals(advertisement.advertiser)) {
  ...
  public void deleteAdvertisement(String title, String advertiserName) {
    advertisementList.removeIf(
            advertisement -> (
                    advertisement.title.equals(title)
